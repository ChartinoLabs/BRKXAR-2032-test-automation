...
DESCRIPTION = (
    ...
    "    The test will verify OSPF neighbor priority values on the following devices: "
...
SETUP = (
    ...
    "{% endif %}\n\n"
    "**OSPF Neighbor Priority Values to Verify:**\n"
    ...
    "    * Expected Priority: {{ neighbor_data.get('priority') }}\n"
    "{% endfor %}"
    "{% endfor %}"
    "{% endfor %}"
)
...
PROCEDURE = (
    ...
    "   * Neighbor Router IDs\n"
    "   * Neighbor Priority Values\n"
    "5. For each device, compare the current OSPF neighbor priority values against expected parameters:\n"
    "{% for device, interfaces in parameters.items() %}"
    "   * Device {{ device }}:\n"
    "{% for interface, interface_data in interfaces.items() %}"
    "     * Interface {{ interface }}:\n"
...
PASS_FAIL_CRITERIA = (
    ...
    "5. Each OSPF neighbor has the correct priority value\n\n"
    ...
    "6. An OSPF neighbor has an incorrect priority value\n\n"
    ...
    "    * Neighbor {{ neighbor_id }} must have priority value \"{{ neighbor_data.get('priority') }}\"\n"
    "{% endfor %}"
    "{% endfor %}"
    "{% endfor %}"
)
...
class VerifyOSPFNeighborsPriority(aetest.Testcase):
    ...
    @aetest.test
    def verify_ospf_neighbors_priority(
